...
KEYWORDS = {
    "أداء المهام الوظيفية": ["تحضير", "شرح", "تنفيذ", "جدول", "مهمة", "وظيفي", "متابعة", "إنجاز", "إعداد", "خطة"],
    "التفاعل الإيجابي مع منسوبي المدرسة والمجتمع": ["زملاء", "فعالية", "مشاركة", "مجتمع", "تعاون", "اجتماع", "فريق", "مجلس", "تواصل", "دعم"],
    "التفاعل مع أولياء الأمور": ["ولي أمر", "تواصل", "رسالة", "لقاء", "اجتماع", "استدعاء", "تقرير", "متابعة", "أولياء", "إشعار"],
    "تنويع استراتيجيات التدريس": ["عصف", "نشط", "مناقشة", "استراتيجية", "طريقة تدريس", "تعاوني", "مجموعات", "استقصاء", "لعب أدوار", "حل مشكلات"],
    "تحسين نواتج التعلم": ["تحسن", "نتائج", "تقدم", "إنجاز", "تحصيل", "نواتج", "تطور", "فعالية", "إتقان", "تعلم"],
    "إعداد وتنفيذ خطة الدرس": ["خطة", "إعداد", "تنفيذ", "أهداف", "مراحل الدرس", "مقدمة", "عرض", "خاتمة", "تحضير", "تسلسل"],
    "توظيف التقنيات والوسائل التعليمية": ["تقنية", "فيديو", "بوربوينت", "حاسب", "عرض", "تفاعلي", "تطبيق", "سبورة ذكية", "منصة", "وسيلة"],
    "تهيئة البيئة التعليمية": ["تهيئة", "صف", "ترتيب", "جو تعليمي", "مقاعد", "إضاءة", "نظافة", "مناسبة", "آمنة", "جاهزة"],
    "ضبط سلوك الطلاب": ["ضبط", "سلوك", "نظام", "التزام", "هدوء", "انضباط", "قواعد", "إجراءات", "تنبيه", "تحفيز"],
    "تحليل نتائج المتعلمين وتشخيص مستواهم": ["تحليل", "تشخيص", "ضعف", "مستوى", "نتائج", "تقارير", "رصد", "اختبار", "مقارنة", "مستهدف"],
    "تنويع أساليب التقويم": ["تقويم", "تقييم", "اختبار", "أداة", "ذاتية", "أوراق عمل", "مقابلة", "شفوي", "عملي", "إلكتروني"]
}
...
# استخراج النص من PDF باستخدام pdf2image و OCR
...
# تحليل استجابة GPT باستخدام الكلمات المفتاحية
def analyze_gpt_response_with_keywords(gpt_text):
    ...
    for i, elem in enumerate(REQUIRED_ELEMENTS):
        ...
        score = 1 if hits == 1 else 2 if hits == 2 else 3 if hits == 3 else 4 if hits == 4 else 5 if hits >= 5 else 1
        if hits >= 2:
            score = 5 if hits >= 2 else 4
        weight = weights[i]
        percent = (score / 5) * 100
        ...
        color = "#d4edda" if score >= 4 else "#fff3cd" if score == 3 else "#f8d7da" if score == 2 else "#f2f2f2"
        note = "إشارات قوية" if score >= 4 else "إشارات ضعيفة" if score == 3 else "مؤشر ضعيف" if score == 2 else "لا توجد إشارات واضحة"
...
# تحليل النص باستخدام GPT ثم تحليل ناتجه بالكلمات المفتاحية
