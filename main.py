...
import requests
import openai
from pdf2image import convert_from_path, convert_from_bytes  # أضفنا convert_from_bytes للتحسين
from PIL import Image
import re
import io  # مطلوب للتحسينات الجديدة
...
def extract_text_from_pdf(pdf_path):
    try:
        # التحسين: استخدام convert_from_bytes بدلاً من العمل مع الملفات مباشرة
        with open(pdf_path, 'rb') as f:
            pdf_data = f.read()
        
        images = convert_from_bytes(pdf_data, dpi=150)  # تقليل DPI لزيادة السرعة
        full_text = ""
        ...
        for i, img in enumerate(images):
            ...
            if os.path.exists(f"page_{i}.jpg"):
...
def process_uploaded_files(request):
    ...
    try:
        ...
        if file and file.filename and file.filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            os.remove(path)  # حذف الصورة بعد معالجتها
            ...
            os.remove(pdf_path)  # حذف ملف PDF بعد معالجته
