...
def calculate_final_score_from_table(gpt_response):
    ...
    if len(scores) < 7:
        return "<div style='color:red; font-weight:bold;'>تعذر عرض الجدول بسبب تنسيق غير صحيح، يرجى إعادة المحاولة بعد التأكد من تنسيق الشاهد.</div>"
...
مهمتك تحليل الشاهد أدناه باستخدام العناصر المعتمدة التالية من وزارة التعليم وعددها 11 عنصرًا، بدون تغيير تسمياتها:
...
ابدأ دائمًا بإخراج جدول HTML بصيغة: <table><tr><td> فقط، يشمل جميع العناصر الـ 11 وبنفس الترتيب.
ثم تحت الجدول، أضف ملاحظات تفصيلية لكل عنصر بهذا الشكل:
